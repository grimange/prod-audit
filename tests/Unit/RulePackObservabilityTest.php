...
use ProdAudit\Audit\Rules\PR_OBS_003_MissingCorrelationIdRule;
use ProdAudit\Audit\Rules\PR_OBS_005_MissingErrorLogsRule;
use ProdAudit\Audit\Rules\RuleInterface;

final class RulePackObservabilityTest extends TestCase
{
    public function testObservabilityPackDetectsFindingsOnBadFixture(): void
    {
        $findings = $this->evaluateRules('stage6/observability_bad.php', [
            new PR_OBS_003_MissingCorrelationIdRule(),
            new PR_OBS_005_MissingErrorLogsRule(),
        ]);

        self::assertNotEmpty($findings);
    }

    public function testObservabilityPackIsCleanOnGoodFixture(): void
    {
        $findings = $this->evaluateRules('stage6/observability_good.php', [
            new PR_OBS_003_MissingCorrelationIdRule(),
            new PR_OBS_005_MissingErrorLogsRule(),
        ]);
